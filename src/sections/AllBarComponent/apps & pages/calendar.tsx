  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">2025</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-1">
          ...
          <div className="mt-6">
            <h2 className="text-lg font-semibold text-gray-700 dark:text-white mb-4">Event Filters</h2>
            <ul className="space-y-2">
              {eventFilters.map((filter) => (
                <li key={filter.name} className="flex items-center">
                  ...
                  />
                  <span className="text-gray-600 dark:text-white">{filter.name}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="mt-6">
            <h2 className="text-lg font-semibold text-gray-700 dark:text-white mb-4">Month</h2>
            <ul className="space-y-2">
              {viewOptions.map((option) => (
                <li key={option} className="text-gray-600 dark:text-white hover:text-blue-600 cursor-pointer">
                  {option}
                </li>
              ))}
            </ul>
        ...
        <div className="md:col-span-3">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-4">September 2025</h2>
          
          <div className="mb-6 grid grid-cols-7 gap-2 text-center font-medium text-gray-700 dark:text-gray-300">
            {daysOfWeek.map((day) => (
            ...
            {calendarDays.map((week, weekIndex) => (
              <div key={weekIndex} className="grid grid-cols-7 gap-2">
                {week.map((day, dayIndex) => (
                  ...
                  >
                    <div className="text-right text-sm font-medium text-gray-700 dark:text-gray-300">{day}</div>
                  </div>
                ))}
              </div>
            ))}
          ...
          <div className="mt-6">
            <h2 className="text-lg font-semibold text-gray-700 dark:text-white mb-4">Month Week</h2>
